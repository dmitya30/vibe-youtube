...
observable demand volume and shape; competition density and incumbent packaging quality; production cost per probe; language and cultural fit; external monetization path strength; policy and reputational risk; measurability of a fast falsification test; durability once crowded; validity at zero channel maturity.

## 5. Comparison table

| cluster | asymmetry type | evidence | cold start valid | independent sources | status |
|---|---|---|---|---|---|
...
| C7 many cheap distinct swings | production cost advantage | CTL-0008, CTL-0013 | yes | 5 | STRONGEST_COLD_START_SUPPORT |
...
C3 was the highest priority and it did not survive scrutiny. The second publication turned out to be the same author republishing the same case, so independence remains unmet, and the newly disclosed fact that the expert already had books, courses and students removes the cold start interpretation entirely. The architecture stays interesting and is now verifiable, since the Telegram channel, the site and a live calculator are all identified.

C6 was reframed rather than rejected. Duration decides which YPP threshold binds once distribution exists, and it actively hurts a channel that has none. This closes a trap that the previous pass left open.

C7 is now the best supported cluster for our situation, with five sources pointing the same way: cold start outcomes are dominated by the number of genuinely different attempts rather than by the polish of any single one.

C8 remains a constraint rather than an opportunity. C10 is new and unverified.
