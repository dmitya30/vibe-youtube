...
Обязательные правила работы ассистента с локальным репозиторием:

1. Пользователь не выполняет серию ручных правок в source files.
2. Изменения передаются одним готовым copy-paste patch для Git Bash /
   MINGW64.
3. Для TSX используются полные блоковые замены по стабильным маркерам
   или полная замена файла.
4. Форматирование-зависимые regex-патчи для TSX запрещены.
5. Все проверки выполняются до записи либо применяется безопасная
   блоковая замена с последующей структурной валидацией.
6. При ошибке Git Bash должен оставаться открытым.
7. Shell `exit` и конструкции `|| exit 1` запрещены.
8. Диагностика patch/check-команд сохраняется через `tee` в `.git/`.
9. Remotion render запускается напрямую и не передаётся через `tee`,
   чтобы сохранить интерактивный progress bar.
10. Lint и render запускаются только после успешного patch и
    `git diff --check`.
11. Production-файл не коммитится и не получает PASS / LOCKED до
    проверки пользователем.
12. После сжатия контекста эти правила восстанавливаются из
    `docs/PLAYBOOK.md` и `docs/REPOSITORY_STRUCTURE.md`.
13. Незакоммиченный файл нельзя реконструировать по памяти: требуется
